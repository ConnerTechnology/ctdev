...
var Registry = []Component{
	...
	{Name: "homelab", Description: "Homelab reverse proxy (Caddy + Cloudflare DNS-01 wildcard)", Category: CategoryInfra, SupportedOS: []OS{OSLinux}, Dependencies: []string{"docker", "sops"}, DetectPath: "$HOME/homelab/docker-compose.yml", GoInstall: homelabInstall, GoUninstall: homelabUninstall, Tags: []string{"homelab", "caddy", "proxy"}},
	...
	{Name: "pihole", Description: "Pi-hole network-wide DNS ad blocker", Category: CategoryInfra, SupportedOS: []OS{OSLinux}, DetectCmd: "pihole", GoInstall: piholeInstall, GoUninstall: piholeUninstall, Tags: []string{"dns", "adblock", "homelab"}},
