...
var Registry = []Component{
	...
	{Name: "beszel", Description: "Beszel server/container monitoring (Docker)", Category: CategoryInfra, SupportedOS: []OS{OSLinux}, Dependencies: []string{"docker"}, DetectPath: "$HOME/beszel/docker-compose.yml", GoInstall: beszelInstall, GoUninstall: beszelUninstall, Tags: []string{"docker", "monitoring", "homelab"}},
	...
	{Name: "caddy", Description: "Caddy reverse proxy (Cloudflare DNS-01 wildcard)", Category: CategoryInfra, SupportedOS: []OS{OSLinux}, Dependencies: []string{"docker"}, DetectPath: "$HOME/caddy/docker-compose.yml", GoInstall: caddyInstall, GoUninstall: caddyUninstall, Tags: []string{"caddy", "proxy", "homelab"}},
	...
	{Name: "nomachine", Description: "NoMachine remote desktop server", Category: CategoryDesktop, SupportedOS: []OS{OSLinux}, DetectPath: "/usr/NX/bin/nxserver", GoInstall: nomachineInstall, GoUninstall: nomachineUninstall, Tags: []string{"remote", "desktop", "vnc"}},
	{Name: "pihole", Description: "Pi-hole network-wide DNS ad blocker (Docker)", Category: CategoryInfra, SupportedOS: []OS{OSLinux}, Dependencies: []string{"docker"}, DetectPath: "$HOME/pihole/docker-compose.yml", GoInstall: piholeInstall, GoUninstall: piholeUninstall, Tags: []string{"dns", "adblock", "homelab"}},
	{Name: "portainer", Description: "Portainer CE Docker management UI (Docker)", Category: CategoryInfra, SupportedOS: []OS{OSLinux}, Dependencies: []string{"docker"}, DetectPath: "$HOME/portainer/docker-compose.yml", GoInstall: portainerInstall, GoUninstall: portainerUninstall, Tags: []string{"docker", "ui", "homelab"}},
	{Name: "restic", Description: "restic backups to B2 + local USB (systemd timer)", Category: CategoryInfra, SupportedOS: []OS{OSLinux}, DetectPath: "/usr/local/bin/restic-backup.sh", GoInstall: resticInstall, GoUninstall: resticUninstall, Tags: []string{"backup", "restic", "homelab"}},
	...
	{Name: "smartmontools", Description: "SMART disk-health monitoring (smartd)", Category: CategorySystem, SupportedOS: []OS{OSLinux}, DetectCmd: "smartctl", GoInstall: smartmontoolsInstall, GoUninstall: smartmontoolsUninstall, Tags: []string{"disk", "health", "homelab"}},
