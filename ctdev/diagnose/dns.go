...
func checkResolvers(ctx context.Context, f Facts) Result {
	if len(f.DNS) == 0 {
		return failf("Reconnect to the network so it hands out DNS servers again, or set 1.1.1.1 manually.",
...
func resolverVerdict(probes []dnsProbe) Result {
	...
	switch {
	case len(working) == 0:
		res := failf("Set DNS to 1.1.1.1 to confirm. If that fixes it, the network's DNS server is down — on a Pi-hole, check the container is running.",
	...
	case len(broken) > 0:
		...
		res := warnf("Remove the dead server from the network's DHCP settings — lookups stall on it before falling back.",
	...
	if slowest.Elapsed > dnsSlowWarn {
		res := warnf("Every page load waits on this before anything else happens. Consider 1.1.1.1 or 9.9.9.9.",
			"%s is slow to answer — %s", slowest.Server, round(slowest.Elapsed))
		res.Data = data
		return res
	}

	res := okf("%s answering (%s)", serverList(working), round(slowest.Elapsed))
	res.Data = data
	return res
}
...
func checkPublicDNS(ctx context.Context, _ Facts) Result {
	for _, s := range publicResolvers {
		...
		if p := queryResolver(ctx, server, dnsProbeName); p.OK() {
			return okf("%s answering (%s)", server, round(p.Elapsed))
		}
	}
	...
	return infof("blocked or unreachable (%s) — normal when a Pi-hole or firewall forces local DNS",
...
func checkDNSHijack(ctx context.Context, f Facts) Result {
	...
	if p.OK() {
		return warnf("Usually an ISP search page or a captive portal. It breaks VPN split-DNS and some installers.",
	...
	if errors.As(p.Err, &dnsErr) && dnsErr.IsNotFound {
		return okf("names that don't exist correctly return no answer")
	}
	return skipf("could not reach %s to test", f.DNS[0])
}
...
func checkSystemDNS(ctx context.Context, _ Facts) Result {
	...
	if err != nil {
		return failf("If individual DNS servers answer but this doesn't, the resolver service on this machine is the problem (systemd-resolved, or a hosts-file override).",
			"the system cannot resolve %s — %s", dnsProbeName, netReason(err))
	}
	return okf("%s resolves in %s (%d addresses)", dnsProbeName, round(elapsed), len(addrs))
}
